| **Category**                            | **Concept / Formula**                                                                                                                        | **Example**                                 | **Answer / Explanation (with context)**                                             |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | ----------------------------------------------------------------------------------- |
| **1. Counting Factors**                 | n = p₁^e₁ × p₂^e₂ × … → **No. of factors = (e₁+1)(e₂+1)…**                                                                                   | 48 = 2⁴ × 3¹                                | Add 1 to each exponent → (4+1)(1+1)=10 → 10 total factors → 1,2,3,4,6,8,12,16,24,48 |
| **Exclude 1 & n**                       | Total − 2 (removes 1 and the number itself)                                                                                                  | 48 → 10−2                                   | 8 factors (all factors except 1 and 48)                                             |
| **Odd Factors Only**                    | Remove all 2’s → apply factor formula                                                                                                        | 70³ = (2×5×7)³ → 5³×7³                      | Only odd part (no 2s): (3+1)(3+1)=16 → 16 odd factors                               |
| **Even Factors Only**                   | Total − Odd                                                                                                                                  | 30⁴ = 2⁴×3⁴×5⁴                              | Total=(4+1)³=125, Odd=(1×5×5)=25 → Even=125−25=100                                  |
| **Product of All Factors**              | Multiply all factors → shortcut: **n^(total/2)**                                                                                             | 66=2×3×11 → total=8                         | Product = 66^(8/2)=66⁴ (since factors come in equal pairs)                          |
| **Prime Factors (with multiplicity)**   | Add all exponents → total count of prime factors (including repeats)                                                                         | 70³ = 2³×5³×7³                              | 3+3+3=9 → means 9 total prime factors when expanded                                 |
| **2. Trailing Zeroes (Product)**        | Count all 2’s & 5’s → **Trailing zeroes = min(count of 2s, 5s)**                                                                             | 16⁶×70¹⁰×95⁶ → (2²⁴)×(2¹⁰×5¹⁰×7¹⁰)×(5⁶×19⁶) | 2’s=24+10=34, 5’s=10+6=16 → min(34,16)=16 trailing zeros                            |
| **3. Trailing Zeroes (Factorial)**      | In n!, zeros come from 5×2 pairs → **Σ⌊n/5ᵏ⌋ until 0**                                                                                       | 400! → ⌊400/5⌋+⌊400/25⌋+⌊400/125⌋           | 80+16+3=99 zeros (each multiple of 5 adds one or more 5s)                           |
| **4. Highest Power of Prime in n!**     | For a prime p, **Σ⌊n/pᵏ⌋ until 0** gives the highest exponent k where pᵏ divides n!                                                          | 7 in 148! → ⌊148/7⌋+⌊148/49⌋                | 21+3=24 → 7²⁴ divides 148!                                                          |
| **5. Highest Power of Composite in n!** | Step 1: Factorize composite → Step 2: Find each prime’s power (using above) → Step 3: Divide each by its exponent → Step 4: Take **minimum** | 12 in 24! → 12=2²×3¹                        | Power(2)=22→22/2=11; Power(3)=10→10/1=10 → min(11,10)=10 → 12¹⁰ divides 24!         |
